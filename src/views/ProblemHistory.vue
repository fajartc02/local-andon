          ...
          <tbody>
            <tr v-for="(problem, i) in containerProblems" :key="problem.fid"
              ...
              <td style="min-width: 140px">
                ...
                <v-btn
                  v-else-if="problem.isLtb && problem.file_report && problem.file_report !== ''"
                  color="blue"
                  small
                  @click="downloadReport(problem)"
                  style="color: white; font-weight: bold;">
                  <i class="fa fa-download" style="margin-right: 4px; width: 100%;"></i> Download Uploaded Report
                </v-btn>
              </td>
            </tr>
          </tbody>
...
export default {
  ...
  methods: {
      ...
      async downloadUploadedReport(problem) {
        ...
        try {
          ...
          if (!/^https?:\/\//i.test(fileUrl)) {
            ...
            const segments = fileUrl.split('/').map(segment => encodeURIComponent(segment));
            ...
            fileUrl = `${baseUrl}/${encodedPath.startsWith('/') ? encodedPath.slice(1) : encodedPath}`;
          ...
          const filename = decodeURIComponent(urlParts[urlParts.length - 1]) || "report.pdf";
    ...
    async downloadReport(problem) {
      try {
        const response = await axios.get(
          `${process.env.VUE_APP_HOST}/v2/download-report?fid=${problem.fid}&problem=${encodeURIComponent(problem.ferror_name)}`,
          { responseType: "blob" }
        );
        const url = window.URL.createObjectURL(new Blob([response.data]));
        const link = document.createElement("a");
