...
export default {
  ...
  mounted() {
    ...
    setTimeout(() => {
      if (this.propsCharData) {
        let isActionAvail = false;
        let containerColors = [];
        let containPointGraph = [
          {
            x: new Date(this.propsCharData[0].clock).getTime() + 1000 * 60 * 60,
        ...
        let currentCondMap = this.propsCharData.map((item, i) => {
          containerColors.push("#19ff30");
          if (item.id_action) {
            isActionAvail = true;
            let objPoint = {
              x:
                new Date(this.propsCharData[i].clock).getTime() +
                1000 * 60 * 60,
              y: this.propsCharData[i].value,
              marker: {
                size: 4,
              },
              label: {
